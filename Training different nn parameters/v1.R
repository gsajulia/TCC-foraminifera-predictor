...
trainingDf <- data.frame(matrix(ncol = 7, nrow = 0))
names(trainingDf)<-c("Hidden neurons", "MSE", "MAE", "RMSE", "RAE", "accuracy", "precision")
for (i in 5:15) {
    for (j in 5:15) {
      print(str_interp("Begin of ${i},${j}"))
        # Neural Network equation
        nn = neuralnet(str_c(outputExpected, " ~ ", formula), data=train,
                    algorithm = "rprop+", startweights = NULL,
                    hidden = c(i, j), stepmax = 1e+06,
        ...
        trainingDf[nrow(trainingDf) + 1,] = c(str_interp("${i},${j}"),
                                                 round(nn$result.matrix[1,], 2),
                                                 round(mae(actual[,1], predicted[,1]), 2),
                                                 round(rmse(actual[,1], predicted[,1]), 2),
                                                 round(rae(actual[,1], predicted[,1]), 2),
                                                 round(accuracy*100, 2),
                                                 round(precision*100, 2))
        print(str_interp("MSE: ${round(nn$result.matrix[1,], 2)}, MAE: ${round(mae(actual[,1], predicted[,1]), 2)},RMSE: ${round(rmse(actual[,1], predicted[,1]), 2)}, RAE: ${round(rae(actual[,1], predicted[,1]), 2)},accuracy: ${round(accuracy*100, 2)}, precision: ${round(precision*100, 2)}"))
        write.csv(trainingDf, str_interp("Training different nn parameters/v1_${outputExpected}.csv"))

...
df <- read.csv("foraminiferos_TMA.csv",
               header = TRUE,
               sep = ",")
...
desnormTest <- data.frame(min(df[, "Temp_mediaAnual"]) + test[, "Temp_mediaAnual"] * (max(df[, "Temp_mediaAnual"]) - min(df[, "Temp_mediaAnual"])))
desnormTest <- data.frame(min(df) + test * (max(df) - min(df)))  

# Neural Network equation
nn = neuralnet(str_c("Temp_mediaAnual", " ~ ", formula), data=train,
...
desnormResult <- data.frame(min(df[, "Temp_mediaAnual"]) + predict$net.result * (max(df[, "Temp_mediaAnual"]) - min(df[, "Temp_mediaAnual"])))
...
results <- data.frame(actual = test$Temp_mediaAnual, prediction = nn.results$net.result)
...
actual=desnormTest$Temp_mediaAnual
